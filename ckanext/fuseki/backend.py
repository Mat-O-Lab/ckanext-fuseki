...
def get_all_named_graphs_from_dataset(dataset_id):
    """
    Query a dataset to find all its named graphs.
    ...
    try:
        ...
        if 'results' in results and 'bindings' in results['results']:
            for binding in results['results']['bindings']:
                if 'g' in binding:
                    graph_uris.append(binding['g']['value'])
        
        log.info(f"Found {len(graph_uris)} named graphs in dataset {dataset_id}")
        return graph_uris
        
    except Exception as e:
        log.error(f"Failed to get named graphs from dataset {dataset_id}: {e}")
        return []
